...
def merge_trs(outer_datadisplaytable):
    ...
    trs = [tr for tr in trs if type(tr) is not NavigableString]
    merged = []
    for tr in trs:
        if tr.select_one('th.ddtitle'):
            merged.append(tr)
        elif tr.select_one('td.dddefault'):
            merged[len(merged)-1] = BeautifulSoup(str(merged[len(merged)-1]) + str(tr), 'lxml')
...
def extract_section_data(section):
    ...
    [section_name, crn, short, section_num] = section.select_one('th.ddtitle').get_text().split(' - ')
    crn = int(crn)
    honors = False
