...
for term_code in reversed(term_codes):
    ...
    for dept in depts:
        ...
        for url in URLS:
            url = url + dept.lower()
            r = requests.get(url)
            # Sometimes a course is offered to undergrads and not grads, & vice versa
            if r.status_code == 200:
                ...
                for course in courses:
                    ...
                    # Sometimes a course if offered in multiple departments, avoid including the other departments
                    if '/' in title:
                    ...
                    if 'to' in hours or '-' in hours:
                        ...
                        credits = int(hours[hours.index(' '):hours.index('.')].strip())

                    # Get how much time is spent in lecture, lab, etc.
                    dist_hours = '. '.join(split_hours[1:])

                    # Get course description
                    description = course.find(class_='courseblockdesc').text.strip()
                    
                    # Get the prerequsites/corequisites of this course
                    prereqs = None
                    if 'Prerequisite:' in description:
                        prereqs = description[description.index('Prerequisite:') + len('Prerequisite:'):].strip()
                    elif 'Prerequisites:' in description:
                        prereqs = description[description.index('Prerequisites:') + len('Prerequisites:'):].strip()
                    if prereqs:
