...
def parse_hours(hours: bs4.BeautifulSoup) -> Tuple[int, int, str]:
    ...
    hours_text = scraper_functions.sanitize(hours_text)
...
def parse_description(courseblockdesc: bs4.BeautifulSoup) -> Tuple[str, str, str]:
    ...
    description_text = scraper_functions.sanitize(description_text)
...
class Command(base.BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for level in education_levels:
            ...
            course_description_html = scraper_functions.request_html(
                description_url)
            url_dept_pairs = collect_departments(course_description_html)
            for path, dept in url_dept_pairs:
                print(dept)
                url = BASE_URL + path
                soup = scraper_functions.request_html(url)
                ...
                for courseblock in courseblocks:
                    ...
                    if results:
                        ...
                        course = course_models.Course(_id=_id,
                                                      dept=dept,
