...
LetterGrades = List[int]
Dept = CourseNum = SectionNum = str
GradeData = NewType(
    'GradeData', Tuple[LetterGrades, Tuple[Dept, CourseNum, SectionNum]])
    ...
    while i < len(text):
        if is_header_row(text[i]):
            ...
            try:
                ...
                letter_grades = section_row[ABCDF_SLICE] + \
                    section_row[ISUQX_SLICE]
                letter_grades = [int(grade) for grade in letter_grades]
                grade_data.append(
                    (letter_grades, (dept, course_num, section_num)))
...
def calculate_gpa(letter_grades: LetterGrades) -> float:
    ...
    F = 0.0
    WEIGHTS = [A, B, C, D, F]
    num_students = sum(letter_grades)
    gpa = 0.0
    for students_with_grade, weight in zip(letter_grades, WEIGHTS):
        gpa += students_with_grade * weight
    return gpa / num_students


def parse_pdf(pdf_reader: PyPDF2.PdfFileReader) -> List[Tuple[GradeData, float]]:
    pdf_data = []
